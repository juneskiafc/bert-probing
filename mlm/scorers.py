...
class MLMScorerPT(BaseScorer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.tokenizer_fast = kwargs.pop('tokenizer_fast') if 'tokenizer_fast' in kwargs else False
        self.mlm_head = kwargs.pop('mlm_head') if 'mlm_head' in kwargs else None

        device = kwargs.pop('device') if 'device' in kwargs else 'cpu'
        super().__init__(*args, **kwargs)
    ...
    def _ids_to_masked(self, token_ids: np.ndarray) -> List[Tuple[np.ndarray, List[int]]]:
        ...
        if self.tokenizer_fast:
            mask_token_id = self._tokenizer._convert_token_to_id_with_added_voc(self._tokenizer.mask_token)
        else:
    ...
    def score(self, corpus: Corpus, temp: float = 1.0, split_size: int = 2000, ratio: float = 0, per_token: bool = False, dataset = None) -> List[float]:
        ...
        # Turn Dataset into Dataloader
        if not self.tokenizer_fast:
            batchify_fn = btf_generic.Tuple(
                btf_generic.Stack(dtype='int32'),
                btf_generic.Pad(pad_val=self._tokenizer._convert_token_to_id(self._tokenizer.pad_token), dtype='long'),
                btf_generic.Stack(dtype='long'),
                btf_generic.Stack(dtype='long'),
                btf_generic.Stack(dtype='long'),
                btf_generic.Stack(dtype='long'))
        else:
            batchify_fn = btf_generic.Tuple(
                btf_generic.Stack(dtype='int32'),
                btf_generic.Pad(pad_val=self._tokenizer._convert_token_to_id_with_added_voc(self._tokenizer.pad_token), dtype='long'),
                btf_generic.Stack(dtype='long'),
                btf_generic.Stack(dtype='long'),
                btf_generic.Stack(dtype='long'),
                btf_generic.Stack(dtype='long'))
        ...
        if per_token:
            ...
            if dataset is not None:
                len_ = len(true_tok_lens)
            else:
                len_ = len(corpus)
            scores = np.zeros((len_,))
        ...
        for batch_id, batch in enumerate(dataloader):
            ...
            for ctx_idx, (sent_idxs, token_ids, valid_length, masked_positions, token_masked_ids, normalization) in enumerate((batch,)):
                ...
                with torch.no_grad():
                    ...
                    if isinstance(self._model.module, AlbertForMaskedLMOptimized) or \
                        ...
                        # input_ids, token_type_ids, attention_mask
                        # this does the whole mlm thing
                        out = self._model.network(
                            input_ids=token_ids,
                            attention_mask=mask,
                            token_type_ids=torch.zeros(token_ids.shape)
                        )
            ...
            if (batch_id+1) % batch_log_interval == 0:
                logging.info("{} sents of {}, batch {} of {}".format(sent_count, len(dataset), batch_id+1, len(batch_sampler)))
                print(f'{sent_count} sents of {len(dataset)}, batch {batch_id+1} of {len(batch_sampler)}')
